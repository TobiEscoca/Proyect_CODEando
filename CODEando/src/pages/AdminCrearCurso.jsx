...
const AdminCrearCurso = () => {
  ...
  const [form, setForm] = useState({
    name: "",
    category: "",
    descripcion: "",   // 👈 tu API lo espera así
    price: 0,
    image: "",
    available: true,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({
        name: "",
        category: "",
        descripcion: "",  // 👈 ojo: acá habías puesto 'decripcion' al resetear
        price: 0,
        image: "",
        available: true,
      });
  ...
  return (
    <div className="max-w-md mx-auto mt-10 text-white">
      ...
      <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="bg-yellow-400 text-gray-900 font-bold py-2 rounded-lg mt-2">
          Crear
        </button>
      </form>
      {message && <p className="mt-2">{message}</p>}

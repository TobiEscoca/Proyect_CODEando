...
export default function MisCursos() {
  ...
  const desinscribirme = async (cursoId) => {
    ...
    setBusyId(cursoId);
    try {
      const r = await authFetch(`${API}/courses/${cursoId}/desinscribirme`, {
        method: "DELETE",
      });
      const d = await r.json();
    ...
    <div className="max-w-3xl mx-auto p-6 text-white">
      ...
      <ul className="space-y-3">
        {rows.map((row) => (
          <li
            key={row.id}
            className="border border-gray-700 rounded p-3 flex gap-3"
          >
            {row.curso?.image && (
              <img
                src={row.curso.image}
            ...
            <div className="flex-1">
              <div className="font-semibold">
                {row.curso?.name || `Curso #${row.curso?.id}`}
              </div>
              <div className="text-xs opacity-70">
                {row.curso?.category || "-"} • {money(row.curso?.price)} •{" "}
                {new Date(row.inscripto_en).toLocaleString()}
              </div>
